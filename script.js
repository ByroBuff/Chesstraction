...
if (activePage.includes("chess.com/game/live/")) {
  var moves = document.querySelector("#live-game-tab-scroll-container > move-list-wc > vertical-move-list")
} else if (activePage.includes("chess.com/game/daily/")) {
  var moves = document.querySelector("#move-list > vertical-move-list")
}

var moveDivs = moves.querySelectorAll('div.move');
url = 'https://lichess.org/import';

moveList = [];


moveDivs.forEach(moveDiv => {
  try {
    const whiteNodeDiv = moveDiv.querySelector('div.white.node');
    const blackNodeDiv = moveDiv.querySelector('div.black.node');

    const whiteNodeContent = whiteNodeDiv.textContent;
    const blackNodeContent = blackNodeDiv.textContent;

    const moveNumber = moveDiv.getAttribute('data-whole-move-number');

    moveList.push(`${moveNumber}. ${whiteNodeContent} ${blackNodeContent}`);
  } catch (error) {
...
chrome.storage.sync.get(
  ...
  (items) => {
    type = items.stringType;
    mode = items.mode;
    console.log(type + " " + mode)
    
if (type == "fen") {
  if (mode == "lichess") {
    open("https://lichess.org/analysis?fen=" + encodeURIComponent(fen))
  } else if (mode == "copy") {
    navigator.clipboard.writeText(fen);
  }
} else if (type == "pgn") {
  if (mode == "lichess") {
    open("https://lichess.org/paste?pgn=" + encodeURIComponent(pgn))
  } else if (mode == "copy") {
